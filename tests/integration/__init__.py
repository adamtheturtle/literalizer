...
``case_discovery``, ``language_specs``, ``literalize_ref_cases``,
``variant_cases``, ``variant_plans``) build and check those
cases. Non-golden tests live alongside this package under ``tests/``
(see ``tests/errors``, ``tests/metadata``, and the top-level modules).
"""
