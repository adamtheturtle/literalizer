...
def _build_variant_cases() -> list[_VariantCase]:
    ...
    # CONST and STATIC declaration styles for Rust produce type-annotated
    # declarations (e.g. ``const x: Vec<&str> = vec![...]``).  The
    # initialisers generated by the default sequence/dict/set formats
    # (vec![], HashMap::from, …) are not const-evaluable, so the golden
    # files would fail the Rust compiler lint.  Exclude them here;
    # dedicated unit tests in test_variables.py cover these styles.
    _rust_const_static_suffixes = ("_const", "_static")
    declaration_style = [
        v
        for v in nv(
        ...
        if not (
            v.lang_cls.__name__ == "Rust"
            and v.name.endswith(_rust_const_static_suffixes)
        )
    ]
    dict_format = nv(
