...
@beartype
def _build_rust_heterogeneous_strategy_variants() -> list[_Variant]:
    """Build Rust-only variants for non-default heterogeneous
    strategies.

    Only Rust exposes ``heterogeneous_strategies``; other languages
    don't have this enum, so the variant list is Rust-specific rather
    than generated by :func:`_build_non_default_variants`.
    """
    default_strategy = Rust.HeterogeneousStrategies.ERROR
    return [
        _Variant(
            name=f"Rust_heterogeneous_strategy_{strategy.name.lower()}",
            spec=Rust(heterogeneous_strategy=strategy),
            lang_cls=Rust,
        )
...
def _build_variant_cases() -> list[_VariantCase]:
    ...
    variant_sources: list[tuple[Iterable[_Variant], str, str]] = [
        ...
        (
            _build_rust_heterogeneous_strategy_variants(),
