...
@beartype
def _build_default_dict_type_variants() -> Iterable[_Variant]:
    """Build default-dict-type variants for languages that support it.

    For each language that advertises ``supports_default_dict_type``,
    create a variant with a non-default value.
    """
    type_overrides: dict[str, str] = {
        "Go": "string",
    ...
    variants: list[_Variant] = []
    for lang_name, lang_config in _LANGUAGES.items():
        if not lang_config.lang_cls.supports_default_dict_type:
            continue
        string_type = type_overrides.get(lang_name, "String")
        variants.append(
            _Variant(
                name=f"{lang_name}_default_dict_type_string",
                spec=lang_config.lang_cls(default_dict_type=string_type),
                wrap=lang_config.wrap,
                wrap_variable_name=lang_config.wrap_variable_name,
            )
        )
...
def _build_variant_cases() -> list[_VariantCase]:
    ...
    variant_sources: list[tuple[Iterable[_Variant], str, str]] = [
        ...
        (_build_default_dict_type_variants(), "empty_dict", ""),
        (_build_default_dict_type_variants(), "simple_dict", ""),
