...
subroutine main_declaration()
    ...
    type(fval_t) :: my_data
    my_data = fmap([fval_t :: &
        fentry('user_name', fint(1_int64)), &
        fentry('user.name', fint(2_int64)), &
        fentry('user-name', fint(3_int64)), &
        fentry('field_name_that_is_really_quite_long_one', fint(4_int64)), &
        fentry('field_name_that_is_really_quite_long_two', fint(5_int64)) &
    ])
