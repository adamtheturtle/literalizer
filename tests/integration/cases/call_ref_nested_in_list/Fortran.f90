...
program main
    ...
    my_data = flist([fval_t :: &
        flist([fval_t :: flist([fval_t :: fmap([fval_t :: fentry('$ref', fstr('my_var'))]), fint(42_int64), fstr('static')])]), &
        flist([fval_t :: flist([fval_t :: fmap([fval_t :: fentry('$ref', fstr('my_other'))]), fint(7_int64), fstr('label')])]) &
    ])
end program main
