...
program main
    ...
    my_data = flist([fval_t :: &
        flist([fval_t :: fmap([fval_t :: fentry('$ref', fstr('repeated_var'))]), fint(1_int64)]), &
        flist([fval_t :: fmap([fval_t :: fentry('$ref', fstr('single_var'))]), fint(0_int64)]), &
        flist([fval_t :: fmap([fval_t :: fentry('$ref', fstr('repeated_var'))]), fint(8_int64)]) &
    ])
end program main
