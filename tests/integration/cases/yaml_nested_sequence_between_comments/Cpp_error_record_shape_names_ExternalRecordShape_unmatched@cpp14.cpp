...
auto my_data = std::vector<std::vector<Value>>{
    std::vector<Value>{
        Value{std::map<std::string, std::string>{{"item", "existing"}}},
        Value{"kept"},
        // This comment trails the first pair.
    },
    std::vector<Value>{Value{std::map<std::string, std::string>{{"item", "next"}}}, Value{"also kept"}},
    // This comment describes the last pair.
    std::vector<Value>{Value{std::map<std::string, std::string>{{"item", "last"}}}, Value{"kept too"}},
};
    (void)my_data;
    return 0;
}
