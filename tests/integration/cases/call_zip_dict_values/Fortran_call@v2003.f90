...
program main
    use fval_m
    implicit none
    call emit(process(fstr('hello')), fmap([fval_t :: fentry('a', fint(1_int64)), fentry('b', fint(2_int64))]))
    call emit(process(fint(42_int64)), fmap([fval_t :: fentry('c', fint(3_int64)), fentry('d', fint(4_int64))]))
contains
    ...
    subroutine emit(call, zip)
        implicit none
        type(fval_t), intent(in) :: call, zip
    end subroutine emit
end program main
