...
program main
    use fval_m
    implicit none
    call process(fstr('hello'), fstr('a'))
    call process(fint(42_int64), fstr('b'))
    call process(fbool(.true.), fstr('c'))
contains
    subroutine process(value, label)
        implicit none
        type(fval_t), intent(in) :: value, label
