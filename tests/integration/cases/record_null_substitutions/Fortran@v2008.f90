...
program main
    ...
    type(fval_t) :: my_data
    my_data = fmap([fval_t :: &
        fentry('rows', flist([fval_t :: fmap([fval_t :: fentry('replacement', fint(-1_int64)), fentry('present', fint(1_int64))]), fmap([fval_t :: fentry('replacement', fint(2_int64)), fentry('present', fint(3_int64))])])) &
    ])
end program main
