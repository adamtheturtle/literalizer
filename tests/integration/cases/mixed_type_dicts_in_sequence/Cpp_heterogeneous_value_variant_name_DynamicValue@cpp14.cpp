...
auto my_data = std::vector<std::map<std::string, DynamicValue>>{
    std::map<std::string, DynamicValue>{{"type", DynamicValue{"create"}}, {"pr_id", DynamicValue{"pr_1"}}, {"draft", DynamicValue{true}}},
    std::map<std::string, DynamicValue>{{"type", DynamicValue{"create"}}, {"pr_id", DynamicValue{"pr_2"}}},
};
    (void)my_data;
    return 0;
}
