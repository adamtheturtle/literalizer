...
def test_rust_const_type_hints_scalars(
    *, json_input: str, expected: str
) -> None:
...
def test_rust_const_type_hint_bytes() -> None:
    """Rust CONST declarations include type annotations for bytes."""
    yaml_input = "!!binary |\n  SGVsbG8="
    result = literalize(
        source=yaml_input,
        input_format=InputFormat.YAML,
...
def test_rust_const_type_hint_date() -> None:
    """Rust CONST with ISO dates uses &str type hint."""
...
def test_rust_const_type_hint_datetime() -> None:
    """Rust CONST with ISO datetimes uses &str type hint."""
...
def test_rust_const_type_hint_set() -> None:
    """Rust CONST declarations include type annotations for sets."""
    yaml_input = "!!set\n? a\n? b\n"
    result = literalize(
        source=yaml_input,
        input_format=InputFormat.YAML,
...
def test_rust_const_type_hint_dict_mixed_values() -> None:
...
def test_rust_const_type_hint_tuple() -> None:
    """Rust CONST with TUPLE format produces tuple type."""
    rust_tuple_const = Rust(
        date_format=Rust.date_formats.ISO,
        datetime_format=Rust.datetime_formats.ISO,
        bytes_format=Rust.bytes_formats.HEX,
    ...
    result = literalize(
        source="[1, 2]",
        input_format=InputFormat.JSON,
        language=rust_tuple_const,
        pre_indent_level=0,
        include_delimiters=True,
        variable_form=NewVariable(name="my_var"),
        error_on_coercion=False,
    )
    assert "const my_var: (i64, i64)" in result.code


def test_rust_const_type_hint_empty_dict() -> None:
...
def test_rust_const_type_hint_homogeneous_list() -> None:
    """Rust CONST with homogeneous integer list uses i64 element type."""
    result = literalize(
        source="[1, 2, 3]",
        input_format=InputFormat.JSON,
        language=RUST_CONST,
