...
@pytest.mark.parametrize(
    argnames="name",
    argvalues=["widget", "My-Widget", "9Widget"],
)
...
def test_cpp_duplicate_shape_names_raises() -> None:
    ...
    with pytest.raises(expected_exception=InvalidRecordNameError):
        Cpp(
            record_shape_names={
                frozenset({"id", "name"}): "Widget",
                frozenset({"id", "title"}): "Widget",
            },
        )
...
@pytest.mark.parametrize(
    argnames="name",
    argvalues=["widget", "My-Widget", "9Widget", "Self"],
)
...
def test_shape_name_collides_with_enum_name_raises() -> None:
    ...
    with pytest.raises(expected_exception=InvalidRecordNameError):
        Rust(
            heterogeneous_value_enum_name="Widget",
            record_shape_names={frozenset({"id", "name"}): "Widget"},
        )
