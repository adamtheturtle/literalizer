...
.. list-table:: Multiline string syntax
   ...
   * - Dart
     ...
     - Escapes interpolation markers, delimiter quotes, and backslashes.
   * - Kotlin
     - Triple-quoted string
   ...
   * - Lua
     - Long-bracket string
     - Chooses a collision-free delimiter; falls back for a leading newline, carriage return, NUL, or source-line trailing whitespace.
   * - Groovy
     - Triple-single-quoted string
     - Escapes backslashes, delimiter quotes, controls, and source-line trailing whitespace without enabling interpolation.
   * - Crystal
     - Non-interpolating percent string
     - Uses a pipe delimiter and falls back for a collision, carriage return, NUL, or source-line trailing whitespace.
   * - D
     - Backtick-delimited WYSIWYG string
     - Falls back for a backtick, carriage return, NUL, or source-line trailing whitespace.
   * - Nim
     - Triple-quoted string
     - Falls back for leading-newline and quote-boundary ambiguity, a delimiter collision, carriage return, NUL, or source-line trailing whitespace.
   * - Swift
...
.. list-table::
   ...
   * - C++
     ...
     - Targets C++14, C++17, or C++20 through ``language_version`` (C++20 is the default). C++14/17 use positional aggregate initialization, ISO date/datetime strings, and ordinary template parameter packs; C++14's explicit ``TUPLE`` or ``RECORD`` strategies render fixed-shape mixed values with native tuples and generated structs.
       ``json_rendering=INLINE_DOCUMENT`` opts into an inline parsed JSON document instead (see below).
   * - Gleam
     - ``GLEAM_JSON_JSON``
