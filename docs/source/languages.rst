...
.. list-table::
   ...
   * - C++
     ...
     - Targets C++14, C++17, or C++20 through ``language_version`` (C++20 is the default). C++14/17 use positional aggregate initialization, ISO date/datetime strings, and ordinary template parameter packs; C++14 requires the explicit ``TUPLE`` or ``RECORD`` heterogeneous strategy for fixed-shape mixed values. Input must not encode the raw-string terminator ``)json"``.
   * - Gleam
     - ``GLEAM_JSON_JSON``
