...
Most backends produce a self-contained file. Ada additionally imports the companion ``A_Stub`` package; copy ``scripts/a_stub.ads`` and ``scripts/a_stub.adb`` beside the generated file before compiling it.
...
Prefer ``wrap_in_file=True`` whenever you need assembled output, adding any companion support files documented by the selected backend.
