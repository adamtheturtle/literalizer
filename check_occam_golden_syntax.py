...
def _check_brackets(lines: list[str]) -> str | None:
    ...
    depth = 0
    for line_num, line in enumerate(lines):  # type: ignore[misc]
        clean = _strip_comment(line)  # type: ignore[misc]
        in_string = False
        for ch in clean:
            if ch == '"':
                in_string = not in_string
                ...
                if ch == "[":
                    ...
                    depth -= 1
                    if depth < 0:
                        return f"Unmatched ']' at line {line_num + 1}"
...
def _check_file(path: Path) -> str | None:
    ...
    top_level_term_indices = [i for i, line in enumerate(lines) if line == ":"]  # type: ignore[misc]
