...
#: Haskell language spec using a custom algebraic data type (ADT).
#:
#: The generated output uses constructors like ``HNull``, ``HBool``,
#: ``HList``, ``HMap``, and ``HSet`` which are **not** built-in Haskell
#: types.  To compile the output, define a ``Val`` ADT and typeclass
...
#:         ...
#:
#: ``OverloadedStrings`` lets bare string literals like ``"hi"`` resolve
#: to ``HStr "hi"`` via ``IsString``, and the ``Num`` / ``Fractional``
#: instances let numeric literals resolve to ``HInt`` / ``HFloat``.
