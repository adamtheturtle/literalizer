...
@dataclasses.dataclass(frozen=True)
class _PreFormState:
    """Shared per-call state that does not depend on ``variable_form``.

    Computed once and reused by ``_literalize_apply_form`` so that
    ``BothVariableForms`` can produce a declaration and an assignment
    without re-parsing or re-formatting the input.
    """
...
def _literalize_apply_form(
    *,
    ...
    """Apply variable-form-specific wrapping to the shared pre-form
    state and return a :class:`LiteralizeResult`.
    """
    result = _apply_variable_wrapper(
        result=pre.base_result,
        language=language,
        data=pre.data,
        variable_form=variable_form,
    )
    if pre.resolved is not None and pre.resolved.pending is not None:
        comment_cfg = language.comment_config
        result = prepend_collection_comments(
            collection_comments=pre.resolved.pending,
            base=result,
            comment_prefix=comment_cfg.prefix,
            comment_suffix=comment_cfg.suffix,
            line_prefix=pre.line_prefix,
        )
    ...
    preamble = (
        ...
        + computed.header
        + language.data_dependent_preamble(pre.coerced_data)
    )
    pre_decl = (
        pre.resolved.pending_scalar_before if pre.resolved is not None else ()
    )
    if wrap_in_file:
        content = result
        if pre_decl:
...
def _literalize_both_forms(
    *,
    ...
    Shares the parse/format/comment-resolution pass between the two
    variable forms — the inputs to those steps don't depend on
    ``variable_form``.
    """
    pre = _literalize_pre_form(
        source=source,
