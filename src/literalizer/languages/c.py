...
def _format_c_ull_literal(value: int) -> str:
    ...
    C's signed integer literals are rejected by the compiler when they
    exceed ``LLONG_MAX``; appending the ``ULL`` suffix selects the
    ``unsigned long long`` type for the literal, which can hold values
    up to ``ULLONG_MAX``.
    """
    if value < 0:
...
class C(metaclass=LanguageCls):
    ...
    def __init__(  # noqa: PLR0915
        ...
        *,
        ...
        _base_int_formatter: Callable[[int], str] = (
