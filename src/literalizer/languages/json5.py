...
class Json5(metaclass=LanguageCls):
    """JSON5 language specification.

    Produces JSON5 values — objects for mappings, and arrays for
    sequences and sets — following the `JSON5 Data Interchange Format
    <https://json5.org/>`_.

    JSON5 is a superset of JSON that permits unquoted identifier keys,
    trailing commas, single- and double-quoted strings, comments, and
    several other syntactic relaxations.
    ...
    language_id: ClassVar[str] = "json5"
    ...
    def format_call_ref_identifier(
        ...
        JSON5 is a data format; values must be literals, not variable
        references, so refs are not supported.
        """

        def _raise_for_json5_ref(name: str, _value: Value | None, /) -> str:
            """Raise ``CallArgNotSupportedError`` unconditionally."""
            raise CallArgNotSupportedError(
                language_name="JSON5",
                reason=(
                    "JSON5 values must be literals, not variable "
    ...
    def format_ordered_map_entry(self) -> Callable[[str, Value, str], str]:
        ...
        return _format_json5_dict_entry
