...
def _format_dart_bigint_literal(value: int) -> str:
    """Format a value that exceeds Dart ``int`` (signed 64-bit) range
    as a ``BigInt.parse(...)`` call.

    Dart's built-in ``int`` is a signed 64-bit integer (on native, and
    a JavaScript double on web); ``BigInt`` is the arbitrary-precision
    escape hatch.
    """
