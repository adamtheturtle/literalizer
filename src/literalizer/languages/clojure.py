...
class Clojure(metaclass=LanguageCls):
    ...
    class DateFormats(enum.Enum):
        ...
        CLOJURE = DateFormatConfig(
            formatter=date_iso_formatter(
                template='#inst "{iso}T00:00:00.000-00:00"',
            ),
    ...
    date_format: DateFormats = DateFormats.CLOJURE
    ...
    def format_call_ref_identifier(
        ...
        Clojure output is not wrapped in a function body, so symbol
        references require a surrounding ``def`` that cannot be
        injected.
        """

        def _raise_for_clojure_ref(name: str, _value: Value | None, /) -> str:
            """Raise ``CallArgNotSupportedError`` unconditionally."""
            raise CallArgNotSupportedError(
                language_name="Clojure",
                reason=(
                    "Clojure output is not wrapped in a function body; "
                    "symbol references require a surrounding ``def`` "
                    f"that cannot be injected (got {name!r})"
                ),
            )
    ...
    def format_variable_assignment(
        ...
        return variable_formatter(template="(def {name} {value})")
