...
# The C standard library and common POSIX function names.  A
# wrapped file imports Foundation, which brings their prototypes
# into scope, so a wrapper of one of these names declares it again
# with a different signature and clang refuses the file with
# ``conflicting types`` (issue #4534).
_LIBC_FUNCTION_NAMES: frozenset[str] = frozenset(
    {
        "abort",
        "abs",
        "acos",
