...
class JsoncTrailingCommas(enum.Enum):
    """JSONC trailing-comma options."""

    NO = TrailingCommaConfig(multiline_trailing_comma=False)


@beartype
def _format_jsonc_dict_entry(
    key: str,
    _raw_value: Value,
    formatted_value: str,
) -> str:
...
class Jsonc(metaclass=LanguageCls):
    """JSONC language specification.

    Produces JSONC values — objects for mappings, and arrays for
    sequences and sets — following the `JSONC specification
    <https://jsonc.org/>`_.

    JSONC adds comments to JSON syntax. Keys are quoted, strings use
    double quotes, and non-finite numbers and trailing commas are rejected.

    Dates and datetimes are rendered as quoted ISO 8601 strings because
    JSONC has no native date type.
    """
    ...
    language_id: ClassVar[str] = "jsonc"
    ...
    class DatetimeFormats(DatetimeFormatEnum):
        """Datetime format options for JSONC."""
    ...
    class SequenceFormats(enum.Enum):
        """Sequence type options for JSONC."""
    ...
    TrailingCommas = JsoncTrailingCommas
    ...
    trailing_comma: JsoncTrailingCommas = JsoncTrailingCommas.NO
    ...
    def format_call_ref_identifier(
        ...
        JSONC is a data format; values must be literals, not variable
        references, so refs are not supported.
        """

        def _raise_for_jsonc_ref(name: str, _value: Value | None, /) -> str:
            """Raise ``CallArgNotSupportedError`` unconditionally."""
            raise CallArgNotSupportedError(
                language_name="JSONC",
                reason=(
                    "JSONC values must be literals, not variable "
                    f"references (got {name!r})"
                ),
            )

        return _raise_for_jsonc_ref
    ...
    def dict_format_config(self) -> DictFormatConfig:
        ...
        return DictFormatConfig(
            ...
            close="}",
            format_entry=_format_jsonc_dict_entry,
            empty_dict=None,
            preamble_lines=(),
            narrowed_open=None,
            supports_trailing_comma=False,
    ...
    def format_string(self) -> Callable[[str], str]:
        ...
        def _format(value: str) -> str:
            """Format a string as a JSONC quoted literal."""
            return (
                format_string_backslash_control(
                    value=value,
    ...
    def format_ordered_map_entry(self) -> Callable[[str, Value, str], str]:
        ...
        return _format_jsonc_dict_entry
    ...
    def format_variable_assignment(
        ...
        return variable_formatter(template="{value}")

    @cached_property
    def scalar_preamble(self) -> dict[type, tuple[str, ...]]:
        """Per-instance scalar preamble (JSONC needs none)."""
        return {}

    @cached_property
    def scalar_body_preamble(self) -> dict[type, tuple[str, ...]]:
        """Per-instance scalar body preamble (JSONC needs none)."""
        return {}
