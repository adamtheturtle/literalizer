...
class Java(metaclass=LanguageCls):
    ...
    def supports_collection_comments(self) -> bool:
        ...
        ``ObjectMapper.readTree`` is handed one JSON document, and JSON
        has no comments, so a comment written inside it would be text
        the parser refuses.  The comments go before the declaration
        instead (issue #4546).
        """
