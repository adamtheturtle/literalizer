...
# The value bindings ``Prelude`` exports, which every module imports
# implicitly.  The wrapped file references its declaration by bare name
# from ``main``, so a declaration taking one of these names makes that
# reference ambiguous and the module does not compile (issue #3942).
_HASKELL_PRELUDE_BINDINGS: frozenset[str] = frozenset(
    {
        "abs",
        "acos",
        "acosh",
...
class Haskell(metaclass=LanguageCls):
    ...
    def supports_collection_comments(self) -> bool:
        ...
        The ``aesonQQ`` quasi-quote holds one JSON document, and
