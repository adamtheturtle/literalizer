...
class Dhall(metaclass=LanguageCls):
    ...
    format_string: ClassVar[Callable[[str], str]] = staticmethod(
        _format_dhall_string
    )
    format_integer: ClassVar[Callable[[int], str]] = staticmethod(
        _format_dhall_integer
    )
    ...
    format_ordered_map_entry: ClassVar[Callable[[str, Value, str], str]] = (
        staticmethod(_format_dhall_dict_entry)
    )
    format_variable_assignment: ClassVar[Callable[[str, str, Value], str]] = (
        staticmethod(
            variable_formatter(template="let {name} = {value} in {name}"),
        )
    )
